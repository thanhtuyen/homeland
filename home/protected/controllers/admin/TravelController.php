    ...
    if(isset($_POST['Travel']))
    {
      ...
      if ($model->validate()) {
        ...
        if($model->save(true,array('title','tieude','content','noidung','image','file','video','create_date','create_user','del_flag','feature_flg', 'is_public')))
          if($ifile_paths){
            uploadMultifile($model, 'image', Travel::image_url);
          }
        if($ffile_paths){
          uploadMultifile($model, 'file', Travel::file_url);
        }

        if($vfile_paths) {
          $model->video->saveAs(Yii::getPathOfAlias('webroot').Travel::video_url . $model->video->name);
          $model->save(false);
        }
    ...
    if(isset($_POST['Travel']))
    {
      ...
      if($model->validate()) {
        ...
        if (is_object($vfile_paths) && get_class($vfile_paths)==='CUploadedFile')
        {
          $model->video = $vfile_paths;
          if($old_video) {
            unlink(Yii::getPathOfAlias('webroot').Travel::video_url . $old_video);
          }
        } else {
...
//      $this->redirect(Yii::app()->homeUrl);
  ...
  public  function actionDelete_image(){
    ...
    if($image_name){
      if($travel[0]['image']){
        $array_file = explode(',', $travel[0]['image']);
        $arr = array_diff($array_file, array($_GET['image_name']));
        $new_file = implode(',', $arr);
        unlink(Yii::getPathOfAlias('webroot').Travel::image_url . $image_name);
        $news = Yii::app()->db
          ->createCommand("UPDATE travel SET `image` ='$new_file' WHERE id=".$id)
          ->execute();
        if($news[0]['image']){
          $array_file = explode(',', $news[0]['image']);
          $string .= '<ul>';
          foreach($array_file as $file)
            $thumb_image_path = Travel::image_url.''.$file;
  ...
  public  function actionDelete_file(){
    ...
    if($file_name){
      if($travel[0]['file']){
        $array_file = explode(',', $travel[0]['file']);
        $arr = array_diff($array_file, array($_GET['file_name']));
        $new_file = implode(',', $arr);
        unlink(Yii::getPathOfAlias('webroot').Travel::file_url . $file_name);
        $news = Yii::app()->db
          ->createCommand("UPDATE travel SET `file` ='$new_file' WHERE id=".$id)
          ->execute();
        if($news[0]['file']){
          $array_file = explode(',', $news[0]['file']);
          $string .= '<ul>';
          foreach($array_file as $file)
            $thumb_file_path = Travel::image_url.''.$file;
      ...
      if($travel[0]['file']){
        ...
        $array_file = explode(',', $travel[0]['file']);
        foreach($array_file as $file){
          $thumb_file_path = Travel::file_url.''.$file;
          $string .=  '<li><input type="checkbox" name="'.$file.'" onclick="delete_file_name(this)"/>'.CHtml::link($file, Yii::app()->createUrl('/travel/downloadFile',array('id' => $id))).'</li>&nbsp';
        }
      }
    }
    $string .= '</ul';
  ...
  {
    $model = $this->loadModel($id);
    $src = Yii::getPathOfAlias('webroot'). Travel::file_url.$model->file;
    if(file_exists($src)) {
