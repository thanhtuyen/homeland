...
//    print_r($_GET);die;
    ...
    {
      $model->attributes=$_POST['Historical'];
      ...
      if ($model->validate()) {
        ...
        if($model->save(true,array('title','tieude','content','noidung','image','file','video','create_date','create_user','del_flag','feature_flg', 'is_public')))
          if($ifile_paths){
            uploadMultifile($model, 'image', Historical::image_url);
          }
        if($ffile_paths){
          uploadMultifile($model, 'file', Historical::file_url);
        }

        if($vfile_paths) {
          $model->video->saveAs(Yii::getPathOfAlias('webroot').Historical::video_url . $model->video->name);
          $model->save(false);
        }
    ...
    if(isset($_POST['Historical']))
    {
      ...
      if($model->validate()) {
        ...
        if (is_object($vfile_paths) && get_class($vfile_paths)==='CUploadedFile')
        {
          $model->video = $vfile_paths;
          if($old_video) {
            unlink(Yii::getPathOfAlias('webroot').Historical::video_url . $old_video);
          }
        } else {
  ...
  {
    $dataProvider=new CActiveDataProvider('Historical');
...
//      $this->redirect(Yii::app()->homeUrl);
    $category_id = "";
      ...
      $category_id = $_GET['category_id'];
    $model=new Historical('search');
    $model->unsetAttributes();  // clear any default values
    if(isset($_GET['Historical']))
      $model->attributes=$_GET['Historical'];
    $model->category_id = $category_id;
   ...
   * @return Historical the loaded model
   * @throws CHttpException
   */
  public function loadModel($id)
  {
    $model=Historical::model()->findByPk($id);
    if($model===null)
   ...
   * @param Historical $model the model to be validated
   */
  protected function performAjaxValidation($model)
  {
    if(isset($_POST['ajax']) && $_POST['ajax']==='Historical-form')
    {
  ...
  public  function actionDelete_image(){
    ...
    $historical = Yii::app()->db->createCommand()
      ->select('*')
      ->from('historical')
      ->where('id='. $id)
      ->andWhere('del_flag = 0')
      ->queryAll();
    if($image_name){
      if($historical[0]['image']){
        $array_file = explode(',', $historical[0]['image']);
        $arr = array_diff($array_file, array($_GET['image_name']));
        $new_file = implode(',', $arr);
        unlink(Yii::getPathOfAlias('webroot').Historical::image_url . $image_name);
        $news = Yii::app()->db
          ->createCommand("UPDATE historical SET `image` ='$new_file' WHERE id=".$id)
          ->execute();
        if($news[0]['image']){
          $array_file = explode(',', $news[0]['image']);
          $string .= '<ul>';
          foreach($array_file as $file)
            $thumb_image_path = Historical::image_url.''.$file;
  ...
  public  function actionDelete_file(){
    ...
    $historical = Yii::app()->db->createCommand()
      ...
      ->queryAll();
    if($file_name){
      if($historical[0]['file']){
        $array_file = explode(',', $historical[0]['file']);
        $arr = array_diff($array_file, array($_GET['file_name']));
        $new_file = implode(',', $arr);
        unlink(Yii::getPathOfAlias('webroot').Historical::file_url . $file_name);
        $news = Yii::app()->db
          ->createCommand("UPDATE historical SET `file` ='$new_file' WHERE id=".$id)
          ->execute();
        if($news[0]['file']){
          $array_file = explode(',', $news[0]['file']);
          $string .= '<ul>';
          foreach($array_file as $file)
            $thumb_file_path = Historical::image_url.''.$file;
      ...
      if($historical[0]['file']){
        ...
        foreach($array_file as $file){
          ...
          $string .=  '<li><input type="checkbox" name="'.$file.'" onclick="delete_file_name(this)"/>'.CHtml::link($file, Yii::app()->createUrl('/historical/downloadFile',array('id' => $id))).'</li>&nbsp';
        }
      }
    }
    $string .= '</ul';
  ...
  {
    $model = $this->loadModel($id);
    $src = Yii::getPathOfAlias('webroot'). Historical::file_url.$model->file;
    if(file_exists($src)) {
