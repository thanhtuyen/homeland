    ...
    if(isset($_POST['Natures']))
    {
      ...
      if ($model->validate()) {
        ...
        if($model->save(true,array('title','tieude','content','noidung','image','file','video','create_date','create_user','del_flag','feature_flg', 'is_public')))
          if($ifile_paths){
            uploadMultifile($model, 'image', Natures::image_url);
          }
        if($ffile_paths){
          uploadMultifile($model, 'file', Natures::file_url);
        }

        if($vfile_paths) {
          $model->video->saveAs(Yii::getPathOfAlias('webroot').Natures::video_url . $model->video->name);
          $model->save(false);
        }
    ...
    if(isset($_POST['Natures']))
    {
      ...
      if($model->validate()) {
        ...
        if (is_object($vfile_paths) && get_class($vfile_paths)==='CUploadedFile')
        {
          $model->video = $vfile_paths;
          if($old_video) {
            unlink(Yii::getPathOfAlias('webroot').Natures::video_url . $old_video);
          }
        } else {
...
//      $this->redirect(Yii::app()->homeUrl);
  ...
  public  function actionDelete_image(){
    ...
    $natures = Yii::app()->db->createCommand()
      ->select('*')
      ->from('natures')
      ->where('id='. $id)
      ->andWhere('del_flag = 0')
    ...
    if($image_name){
      if($natures[0]['image']){
        $array_file = explode(',', $natures[0]['image']);
        $arr = array_diff($array_file, array($_GET['image_name']));
        $new_file = implode(',', $arr);
        unlink(Yii::getPathOfAlias('webroot').natures::image_url . $image_name);
        $news = Yii::app()->db
          ->createCommand("UPDATE natures SET `image` ='$new_file' WHERE id=".$id)
          ->execute();
        if($news[0]['image']){
          $array_file = explode(',', $news[0]['image']);
          $string .= '<ul>';
          foreach($array_file as $file)
            $thumb_image_path = Natures::image_url.''.$file;
  ...
  public  function actionDelete_file(){
    ...
    if($file_name){
      if($natures[0]['file']){
        $array_file = explode(',', $natures[0]['file']);
        $arr = array_diff($array_file, array($_GET['file_name']));
        $new_file = implode(',', $arr);
        unlink(Yii::getPathOfAlias('webroot').Natures::file_url . $file_name);
        $news = Yii::app()->db
          ->createCommand("UPDATE natures SET `file` ='$new_file' WHERE id=".$id)
          ->execute();
        if($news[0]['file']){
          $array_file = explode(',', $news[0]['file']);
          $string .= '<ul>';
          foreach($array_file as $file)
            $thumb_file_path = Natures::image_url.''.$file;
      ...
      if($natures[0]['file']){
        ...
        $array_file = explode(',', $natures[0]['file']);
        foreach($array_file as $file){
          $thumb_file_path = Natures::file_url.''.$file;
          $string .=  '<li><input type="checkbox" name="'.$file.'" onclick="delete_file_name(this)"/>'.CHtml::link($file, Yii::app()->createUrl('/natures/downloadFile',array('id' => $id))).'</li>&nbsp';
        }
      }
    }
    $string .= '</ul';
  ...
  {
    $model = $this->loadModel($id);
    $src = Yii::getPathOfAlias('webroot'). Natures::file_url.$model->file;
    if(file_exists($src)) {
