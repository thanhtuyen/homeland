...
<div id="wrapper">
  ...
  <div class="gallery">
    ...
    <video  width="625" height="350" id="player2" controls="controls">
      <source  src="<?php echo Yii::app()->request->baseUrl.Economy::video_url.''.$model->video;?>" type="video/mp4">
    </video>
    <?php endif;?>
    <?php
    $array_file = explode(',', $model->image);
    if($model->image):?>
    <img style="width: 500px;height: 500px;" src="<?php echo Economy::image_url.''.$array_file[0];?>" alt="" class="main-img" />
    <ul>
      <?php
      foreach($array_file as $file) {
        $thumb_image_path = '/uploadfile/economy/images/'.''.$file;
...
<div class="file">
  ...
  if($model->file){
    ...
    foreach($array_file as $file){
      ...
      echo CHtml::link($file, Yii::app()->createUrl('/economy/downloadFile',array('id' => $model->id)));
    }
  }
