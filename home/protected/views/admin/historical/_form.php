...
<input type="hidden"id="historical_id" value="<?php echo  $model->id;?>">
<div class="form">
  <?php
...
//    print_r($parents[$category_id] );die;
  ...
  echo '<div class="form-group">';
    echo'<label class="col-sm-3 control-label" for="historical_image">Image</label>';
  ...
  echo '<div class="form-group">';
    echo'<label class="col-sm-3 control-label" for="historical_image">File</label>';
    echo '<div class="col-sm-9">';
    $this->widget('CMultiFileUpload', array(
      'model' => $model,
      'attribute' => 'file',
...
//  }
  ...
  $video = Yii::app()->request->baseUrl.Historical::video_url.''.$model->video;
  if($model->video){?>
 <div class="form-group">
   <label class="col-sm-3 control-label" for="historical_image"></label>
   <div class="col-sm-9">
...
<script>
  var id = $('#historical_id').val();
  $.ajax( {
    type: 'GET',
    url: '../../historical/delete_image',
    data: "id="+id,
  ...
  function delete_image_name(elm){
    ...
    if (result==true) {
      $.ajax( {
        ...
        success: function(data) {
          $( "#gallery" ).load( "/admin.php/historical/delete_image?id="+id, function() {
  ...
  function delete_file_name(elm){
    ...
    if (result==true) {
      $.ajax( {
        ...
        success: function(data) {
          $( "#gallery_file" ).load( "/admin.php/historical/delete_file?id="+id, function() {
