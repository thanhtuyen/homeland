...
<input type="hidden"id="culture_id" value="<?php echo  $model->id;?>">
<div class="form">
  <?php
...
//    print_r($parents[$category_id] );die;
  ...
  echo '<div class="form-group">';
    echo'<label class="col-sm-3 control-label" for="culture_image">Image</label>';
  ...
  echo '<div class="form-group">';
    echo'<label class="col-sm-3 control-label" for="culture_image">File</label>';
    echo '<div class="col-sm-9">';
    $this->widget('CMultiFileUpload', array(
      'model' => $model,
      'attribute' => 'file',
...
//  }
  ...
  $video = Yii::app()->request->baseUrl.Culture::video_url.''.$model->video;
  if($model->video){?>
 <div class="form-group">
   <label class="col-sm-3 control-label" for="culture_image"></label>
   <div class="col-sm-9">
...
<script>
  var id = $('#culture_id').val();
  $.ajax( {
    type: 'GET',
    url: '../../culture/delete_image',
    data: "id="+id,
  ...
  function delete_image_name(elm){
    ...
    if (result==true) {
      $.ajax( {
        ...
        success: function(data) {
          $( "#gallery" ).load( "/admin.php/culture/delete_image?id="+id, function() {
  ...
  function delete_file_name(elm){
    ...
    if (result==true) {
      $.ajax( {
        ...
        success: function(data) {
          $( "#gallery_file" ).load( "/admin.php/culture/delete_file?id="+id, function() {
